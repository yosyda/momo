...
SDL は高度に抽象化された GUI ライブラリーではありません。  
むしろ、 GUI ライブラリーを実装するためのライブラリーといった位置づけが近いのではないかと思います。  
もちろん、ボタンに対応するコンポーネントも存在しないので、ボタンを表現する方法を自分で考える必要があります。  
今回は以下の2つの方法を検討しました。
...
今回は、以下の理由から SDL_image を利用することにしました。

- 依存が少なくビルドへの追加が簡単そうだった
- (文字を描画する場合と比較して、) 画像を利用してリッチなボタンを表現できる
...
また、サンプル・ブランチには以下の機能も実装してあるので参考にしてください。
